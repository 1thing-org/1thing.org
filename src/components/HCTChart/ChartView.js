...
const ChartView = () => {
  ...
  const loadData = () => {
    ...
    incidentsService.getStats(dateRange[0], dateRange[1]).then((stats) => {
      const mergedData = mergeDate(stats.stats, dateRange[0], dateRange[1], stats.monthly_stats)
  ...
  return (
    <div className="hcr">
      <div className="title">
        ...
        <p className="t4">The following numbers are based on incidents reported by major media in the US:</p>
      </div>
      ...
      <div className="chart">
        <IncidentChart className="behind-relative" chart_data={incidentTimeSeries} isFirstLoadData={isFirstLoadData} />
      </div>

      <div>
        <Row className="link1">
          <Col lg={{ offset: 10 }} md={{ offset: 8 }} sm={{ offset: 5 }} xs={{ offset: 5 }}>
            <a className="vm1" href="https://hatecrimetracker.1thing.org/" target="_blank" rel="noopener noreferrer">
              View More &rarr;
            </a>
          </Col>
        </Row>
